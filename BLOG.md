...
- **And one third mispredicted case**, found while consolidating my serving-config presets: I assumed bumping `MAX_MODEL_LEN` from 16k to 64k would be free for short-prompt workloads, since paged attention is supposed to make it a per-request cap. Measurement said no — batch req/s collapsed from 21.1 to 13.9 (−34%), same magnitude as `BLOCK_SIZE=32`. Probable mechanism: vLLM V1 reserves KV / CUDA-graph capacity per in-flight slot in proportion to `max_model_len`, so it behaves as a worst-case-per-slot reservation, not a request cap. Three for three on "argue from mechanism, get burned by measurement."
